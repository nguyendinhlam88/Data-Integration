...
class BonbanhSpider(scrapy.Spider):
    ...
    def parse_ad(self, response):
        ...
        # item['mo_ta'] = response.xpath('//*[contains(concat( " ", @class, " " ), concat( " ", "des_txt", " " ))]//text()').extract()
        item['thuoctinh_1'] = response.xpath('//h1/text()').extract_first()
        info = response.xpath('//span[@class = "inp"]')
        item['thuoctinh_2'] = info[0].xpath('.//text()').extract_first()
        item['thuoctinh_3'] = info[1].xpath('.//text()').extract_first()
        item['thuoctinh_4'] = info[2].xpath('.//text()').extract_first()
        item['thuoctinh_5'] = info[3].xpath('.//text()').extract_first()
        item['thuoctinh_6'] = info[4].xpath('.//text()').extract_first()
        item['thuoctinh_7'] = info[6].xpath('.//text()').extract_first()
        item['thuoctinh_8'] = info[7].xpath('.//text()').extract_first()
        item['thuoctinh_9'] = info[8].xpath('.//text()').extract_first()
        item['thuoctinh_10'] = info[10].xpath('.//text()').extract_first()
        item['thuoctinh_11'] = response.xpath('//*[contains(concat( " ", @class, " " ), concat( " ", "des_txt", " " ))]//text()').extract()
